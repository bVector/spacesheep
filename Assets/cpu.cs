...
//        Vector3 origin = transform.position;
    ...
    bool CreateGun(int x, int y)
    {
        ...
        GameObject tobj = (GameObject)Instantiate(GunPrefab, pos, GetComponent<Rigidbody2D>().transform.rotation);

        tobj.transform.parent = transform;

        GameObject NewArmor = (GameObject)tobj;

        Gun gun = NewArmor.GetComponent<Gun>();

        bool createsuccess = map.AddBlock(x, y, gun);
        if (!createsuccess)
            throw null;
        /*
        foreach (Block block in blocks)
